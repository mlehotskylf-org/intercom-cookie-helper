...
type Config struct {
	...
	// Note: IntercomJWTSecret and IntercomJWTTTL removed - JWT is now generated by Auth0 Action
...
func FromEnv() (Config, error) {
	...
	// Note: INTERCOM_JWT_SECRET and INTERCOM_JWT_TTL no longer needed - JWT is generated by Auth0 Action
...
func (c *Config) Validate() error {
	...
	// INTERCOM_JWT_TTL validation removed - JWT is now generated by Auth0 Action
	...
	if c.Env == "prod" {
		// INTERCOM_JWT_SECRET no longer used - JWT generated by Auth0 Action
		if c.Auth0ClientSecret != "" {
			return fmt.Errorf("in prod, AUTH0_CLIENT_SECRET must be unset (use secret manager instead)")
		}
	} else {
		// In dev/staging, Auth0 client secret is required
		// INTERCOM_JWT_SECRET no longer required - JWT generated by Auth0 Action
		if c.Auth0ClientSecret == "" {
...
// The JWT is now provided by Auth0 Action, so no secret or TTL is needed here.
