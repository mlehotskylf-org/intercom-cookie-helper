	...
	tests := []struct {
		...
		},
		{
			...
			config: Config{
				...
				IntercomJWTSecret: []byte("should-not-be-set"),
		...
		},
		{
			...
			config: Config{
				...
				Env:               "dev",
				IntercomJWTSecret: []byte("secret"),
				IntercomJWTTTL:    3 * time.Minute, // Too short (min 5m)
				Auth0ClientSecret: "secret",
				RedirectTTL:       30 * time.Minute,
				SessionTTL:        24 * time.Hour,
				LogLevel:          "info",
			},
			expectedErr: "INTERCOM_JWT_TTL must be between 5m and 30m, got 3m0s",
		},
		{
			name: "IntercomJWTTTL too long",
			config: Config{
				AppHostname:       "example.com",
				Port:              "8080",
				CookieDomain:      ".example.com",
				...
				Env:               "dev",
				IntercomJWTSecret: []byte("secret"),
				IntercomJWTTTL:    45 * time.Minute, // Too long (max 30m)
				Auth0ClientSecret: "secret",
				RedirectTTL:       30 * time.Minute,
				SessionTTL:        24 * time.Hour,
				LogLevel:          "info",
			},
			expectedErr: "INTERCOM_JWT_TTL must be between 5m and 30m, got 45m0s",
		},
	}
