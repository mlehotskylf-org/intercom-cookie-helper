...
func handleCallback(w http.ResponseWriter, r *http.Request) {
	...
	log.Printf("User info parsed from ID token - email_present: %v, name_present: %v, intercom_jwt_present: %v",
		userInfo.Email != "", userInfo.Name != "", userInfo.IntercomJWT != "")
	...
	// Step 13: Render Intercom identify page with JWT from Auth0
	// Build the identify payload with user information and pre-generated JWT
	payload := auth.IdentifyPayload{
		ReturnTo:    returnTo,
		Email:       userInfo.Email,
		Name:        userInfo.Name,
		IntercomJWT: userInfo.IntercomJWT, // JWT generated by Auth0 Action
	}
	...
	metrics.CbOK.Add(1)
	log.Printf("Identify rendered - has_email: %v, return_host: %s",
		userInfo.Email != "", returnHost)
}
